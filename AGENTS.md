...
- Claude code is an AI agent, it can follow instructions provided, writes code, executes cmds, just like you it can do whatever its asked to do.
- Its faster to write code, so anytime there is a need to write code, you can instruct claude code.
- Claude code runs in tmux session codex-review.1, use `tmux capture-pane -pt codex-review.1` to check its current state. Use `tmux send-keys -t codex-review.1 'instructions' Enter` to tell anything to it.
- Always run capture-pane before sending instructions because you have to see what claude code is doing and decide if you want to send instructions or not
- You can send Ctrl-c to cancel anything running, if by mistake you kill the claude code session, you can always run it with `claude --dangerously-skip-permissions`
